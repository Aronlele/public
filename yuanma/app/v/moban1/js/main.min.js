...
(function(b, f, i) {
    ...
    function j() {
        var a = b(this), d;
        a.is(":password") || (a.data("password") ? (d = a.next().show().focus(),
    ...
    function k() {
        ...
        b.trim(a.val()).length > 0 || (a.is(":password") ? (c = a.attr("id") + "-clone",
        d = b("<input/>").attr(b.extend(m(this), {
            type: "text",
            value: placeholder,
            "data-password": 1,
            id: c
        ...
        a.before(d).hide(),
        b("label[for=" + a.attr("id") + "]").attr("for", c)) : (a.val(placeholder),
            ...
            if (null  != b) {
                ...
                d.each(a.axis.split(""), function(b, d) {
                    var c = "x" == d ? "Left" : "Top"
                      ...
                      , h = l[f]
                      , r = n.max(l, d);
                    p ? (e[f] = p[m] + (s ? 0 : h - k.offset()[m]),
                    a.margin && (e[f] -= parseInt(g.css("margin" + c)) || 0,
                    e[f] -= parseInt(g.css("border" + c + "Width")) || 0),
                    e[f] += a.offset[m] || 0,
                    a.over[m] && (e[f] += g["x" == d ? "width" : "height"]() * a.over[m])) : (c = g[m],
                    e[f] = c.slice && "%" == c.slice(-1) ? parseFloat(c) / 100 * r : c);
                    a.limit && /^\d+$/.test(e[f]) && (e[f] = 0 >= e[f] ? 0 : Math.min(e[f], r));
                    !b && a.queue && (h != e[f] && q(a.onAfterFirst),
                    delete e[f])
                });
                q(a.onAfter)
            }
...
(function(e) {
    ...
    function s(e, s, u) {
        ...
        if (e) {
            ...
            if (a.lastIndexOf("_") > 0) {
                var f = a.substring(0, a.lastIndexOf("_") + 1)
            }
            var l = e.id;
            if (!l) {
...
$(document).ready(function() {
    ...
    $.ajax({url:'/api/user/loginstate',success:function(data){
            $('#member-links-head').html(data);
        },async:false});
    ...
    ,
    function() {
        var getByKey = function (c_name, ss)
        {
            var str = document.cookie; 
            if(ss)  str = decodeURI(ss);
        if (str.length>0)
          {
        ...
        var user = {};
        user.show = function() {
            var $root =  $(".member-links");
            $root.find("[data-fancybox = 'ajax']") .eq(0).text(this.name);
            $root.find("[data-fancybox = 'ajax']"). eq(1).text(this.score);
            $root.show();
        }
        user.hide = function() {
            $(".member-links").show();
        }
        user.set = function(name, score) {
            this.name = name;
            this.score = score;
        }
        ...
        login.check = function() {
            var name = getByKey("username", getByKey("data"));
            var score = getByKey("score", getByKey("data"));  
            if (name.length > 0)  {
                user.set(name, score);
                return true;
            } else return false;
        ...
        var a = {}
          ...
          , f = function(a, c) {
            if (b && b[c])
                for (var d in b[c])
                    b[c].hasOwnProperty(d) && (a[d] = b[c][d]);
            return a
        }
          , g = function(a) {
                ...
                for (var d = 0; d < c.length; d++) {
                    ...
                    if (e.length == 2) {
                        ...
                        for (var g = 0; g < f.length; g++)
                            b[f[g]] = decodeURIComponent(e[1]).replace(/[+]/g, " ")
                    }
                }
            }
            return b
        }
          , j = function(b, c, d, e) { //ajax请求数据。
            var f = d.url ? d.url : window.location.href;
            f.indexOf("#") > 0 && (f = f.substring(0, f.indexOf("#"))),
            $.ajax({
                url: f + (f.indexOf("?") >= 0 ? "&" : "?") + "method=ajax&block_id=" + b + (e ? "&" + $.param(e) : ""),
                type: "GET",
                cache: !1,
                beforeSend: function() {
                    $(c).block({
         ...
         */
          , l = function(c, d) {
            ...
            c.ajaxForm({
                ...
                beforeSubmit: function(a, form) {  
                    // a是表单的数据 是一个数组 如[ { name: 'username', value: 'jresig' }, { name: 'password', value: 'secret' } ]

                    var formVerifiedOK = true;
                    a.forEach(function(item)  { 
                         var legal = signUpVerify({
                            value: item.value,
                            field: item.name,
                            $errHolder: form.find("[name="+item.name+"]").siblings(".field-error"),
                            form: form
                         });
                         !legal && (formVerifiedOK = false);
                    });
                  // console.log("formVerifiedOK" + formVerifiedOK);
                   if(!formVerifiedOK) return false;
                ...
                /* 注册成功返回结果
                 * a[json|string]: status[string],errors[arr],data,redirect
                 */
                    ...
                    if (g.getResponseHeader("Content-Type").indexOf("application/json") >= 0) {
                        ...
                        if (a.status == "failure") {
                            for (var i = 0; i < a.errors.length; i++) {
                                ...
                                if (!n || !n.exists())
                                    n = c.find(".generic-error");
                                n.empty().text(m).fadeIn();
                                //验证码重置
                                if (k == "code" && l != "required") {
                                    var q = c.find(".captcha-control img");
                                    q.exists() && (q.attr("src", q.attr("src").replace(new RegExp("rand=\\d+"), "rand=" + (new Date).getTime())),
                                    c.find(".captcha-control .textfield").val(""))
                                }
                            }
                            d && d.error && d.error(c),
                            h(c, 0)
                        ...
                        g.getResponseHeader("Content-Type").indexOf("text/html") >= 0 ? d && d.success ? d.success(c, a) : $(a).attr("data-fancybox") == "message" || $(a).find('[data-fancybox="message"]').exists() ? $.fancybox($(a), {
                            topRatio: .3,
                        ...
                        }) : c.empty().append(a) : (c.find(".generic-error").text(f).show(),
                        h(c, 0),
                        d && d.error && d.error(c))
                },
                error: function() {
            ...
            c.find(".list-selector").each(function() {
                ...
                a.find('[type="text"]').focus(function() {
                    ...
                    a.find(".list-selector-popup").exists() ? (a.find(".list-selector-popup").show(),
                    a.find('[name="filter"]').focus()) : c && d && $.ajax({
                        url: c,
                        type: "GET",
                        beforeSend: function() {
                            b.block({
                        ...
                        success: function(c) {
                            ...
                            i.find('input[type="radio"]').change(function() {
                                ...
                                if (h.html()) {
                                    var j = c.prop("value");
                                    c.prop("checked") && (a.find('input[type="hidden"][name="' + d + '[]"]').remove(),
                                    f[0] = j,
                                    g[0] = h.html(),
                                    a.append($('<input type="hidden" name="' + d + '[]" value="' + j + '"/>')));
                                    var k = g[0];
                            ...
                            j.focus().keyup(function() {
                                j.val() != h && (h = this.value.toLowerCase(),
                ...
                $(this).click(function() {
                    if (!$(this).hasClass("disabled")) {
                        ...
                        if (b.prop("checked")) {
                            ...
                            c.exists() && (b.attr("name") == "payment_option" ? (c.append(c.parent().find('[type="submit"]')),
                            c.parent().find("label").removeClass("hidden"),
                            c.removeClass("hidden")) : b.attr("name") == "card_package_id" && (c.parent().append(c.find('[type="submit"]')),
                            c.parent().find("label").addClass("hidden"),
                            c.addClass("hidden")))
                        }
                    }
                })
            ...
            c.find("input").blur(function() {
               var val = $(this).val(),
                      ...
                      $holder = $(this).siblings(".field-error");
               var legal = signUpVerify({
                    value: val,
                    field: field,
                    $errHolder: $holder ,
          ...
          , n = function(c, d, e, f) {//ajax
            if (!c)
                ...
                i.error = function() {
                    ...
                    var a = !1;
                    for (var b in g)
                        g.hasOwnProperty(b) && b.indexOf("from") == 0 && parseInt(g[b]) > 1 && (g[b] = parseInt(g[b]) - 1,
                        k > 1 && (g[b] = 1),
                        a = !0);
                    a || (g = null ),
                    k > 1 && delete i.error,
            ...
            $(".btn-favourites li a").click(function(a) {
                ...
                if (c || d)
                    ...
                    b.hasClass("delete") ? k(b.parents("li"), {
                        ...
                        a.status == "success" && (g > 0 ? (b.parents("li").addClass("hidden"),
                        b.parents("ul").find("#add_playlist_" + g).removeClass("hidden")) : (b.parents("li").addClass("hidden"),
                        b.parents("ul").find("#add_fav_" + e).removeClass("hidden")))
                    }) : e == 10 && !g ? f && m(b, f, function() {
                        ...
                        a.status == "success" && (g > 0 ? (b.parents("li").addClass("hidden"),
                        b.parents("ul").find("#delete_playlist_" + g).removeClass("hidden")) : (b.parents("li").addClass("hidden"),
                        b.parents("ul").find("#delete_fav_" + e).removeClass("hidden")))
                    })
            })
        }
          , u = function() {
          ...
          , v = function() {
            var a = $(".block-share");
            a.exists() && (typeof window.getEmbed == "function" && a.find(".embed-code").val(window.getEmbed()),
            a.find(".embed-button").click(function(b) {
                b.preventDefault();
            ...
            a.find(".embed-code-custom-width, .embed-code-custom-height").on("keyup blur", function() {
                ...
                parseInt(b.val()) && parseInt(c.val()) && (a.find(".embed-code-custom-width-label").html(parseInt(b.val())),
                a.find(".embed-code-custom-height-label").html(parseInt(c.val())),
                a.find(".embed-button-custom").attr("data-embed-size", parseInt(b.val()) + "x" + parseInt(c.val())))
            }))
        }
          , w = function() {
            ...
            if (c.exists() && d.exists()) {
                ...
                e.exists() && l(e, {
                    success: function(f, g) {
                        ...
                        c.find(".toggle-button").exists() && (d.hide(),
                        c.find(".toggle-button").removeClass("active"));
          ...
          , x = function() {
            $(".list-comments").on("click", ".comment-options a", function(a) {
                var c = $(this)
                  , d = $(this).parents(".item")
                    ...
                    k(c, {
                        ...
                        if (a.status == "success") {
                            ...
                            if (f.exists()) {
                                ...
                                isNaN(b) || (b += m,
                                f.html(b),
                                b > 0 ? f.addClass("positive") : b < 0 ? (f.addClass("negative"),
                                d.addClass("dim-comment")) : b == 0 && (f.removeClass("positive").removeClass("negative"),
                                d.removeClass("dim-comment")))
                            }
                        } else
                            h.addClass("disabled")
                    })
                } else
                    c.hasClass("comment-edit") && (a.preventDefault(),
          ...
          , z = function(g) {
                ...
                return function(d) {
                    if (d.status == "success") {
                        ...
                        if (a.attr("data-refresh-block-ids")) {
                            ...
                            b.attr("data-redirect-url") && (window.location = b.attr("data-redirect-url"))
                    } else {
                    ...
                    for (var q in o)
                        if (o.hasOwnProperty(q)) {
                            ...
                            if (s) {
                                ...
                                d[q] && ($("#" + q).find("img.lazy-load").removeClass("lazy-load"),
                                n(q, q, !1, !1))
                            }
                        }
                }
            } else
                g.find('[data-fancybox="ajax"]').each(function() {
                    $(this).click(function(a) {
                        a.preventDefault(),
                        m($(this), this.href || $(this).attr("data-href"))
                    })
                    ...
                    c.success = function(a, b) {
                        ...
                        if (f) {
                            ...
                            g.fadeIn().promise().done(function() {
                                for (var a in n)
                                    if (n.hasOwnProperty(a) && a.indexOf("from") == 0 && parseInt(n[a]) > 1) {
                                        delete n[a];
                                        break
                                    }
                                d[k] || typeof Storage != "undefined" && (sessionStorage.setItem(o + location.href + "#" + k, $("#" + k).html()),
                                sessionStorage.setItem(o + location.href + "#" + k + ":params", JSON.stringify(n)))
                            }),
                            z(g)
                        } else
                            h($("#" + k)),
                            $("#" + k).animate({
                                ...
                                z($("#" + k)),
                                typeof Storage != "undefined" && (sessionStorage.setItem(o + location.href + "#" + k, $("#" + k).html()),
                                sessionStorage.setItem(o + location.href + "#" + k + ":params", JSON.stringify(n)))
                            })
                    }
                    ,//调用ajax
            ...
            if (!a.playlist_state) {
                ...
                g.find("[data-playlist-item]").each(function() {
            ...
            g.find('[data-action="select"]').each(function() {
                $(this).click(function(a) {
                    if (!$(this).hasClass("disabled")) {
                        ...
                        c.prop("checked") ? ($(this).addClass("active"),
                ...
                $(this).click(function() {
                    if (!$(this).hasClass("disabled")) {
                        var a = $(this).parents("form")
                          , b = $(this).find("input");
                        b.prop("checked", !0),
                        b.prop("checked") && (a.find('[data-action="choose"]').removeClass("active"),
                        $(this).addClass("active"))
                    }
                })
            }),
            ...
            g.find("form[data-controls]").each(function() {
                ...
                a.find('input[type="button"]').each(function() {
                    $(this).click(function(c) {
                                ...
                                for (var e = 0; e < d.length; e++) {
                                    ...
                                    if (g.length == 1)
                                        ...
                                        for (var i = 0; i < h.length; i++) {
                                            var j = h[i].trim();
                                            if (j.indexOf("//") == 0) {
                                                if (f % 100 == parseInt(j.substring(2)))
                                                    return g[1].trim().replace("%1%", "" + f)
                                            } else if (j.indexOf("/") == 0) {
                                                if (f % 10 == parseInt(j.substring(1)))
                                                    return g[1].trim().replace("%1%", "" + f)
                                            } else if (f == parseInt(g[0].trim()))
                                                return g[1].trim().replace("%1%", "" + f)
                                        }
                                    }
                                }
                        ...
                        var g = {};
                        if (!e || confirm(e))
                            if (d.attr("data-action") == "select_all")
                                d.hasClass("active") ? a.find("input[type=checkbox]").each(function() {
                                    this.checked && $(this).click()
                                }) : a.find("input[type=checkbox]").each(function() {
                                    this.checked || $(this).click()
                                });
                            else if (d.attr("data-action") == "delete_multi")
                                g = i(a.attr("data-parameters")),
                                g["function"] = "get_block",
                                g.block_id = b,
                                g["delete"] = [],
                                a.find("input[type=checkbox]").each(function() {
          ...
          , C = function() {
            ...
            $("[data-search-filter-ids] input").click(function() {//搜索条件选择框
                ...
                c.find("input").each(function() {
                    ...
                    if (a.prop("type") == "hidden" || a.prop("type") == "checkbox" && a.prop("checked"))
                        if (a.prop("name").indexOf("[]") > 0) {
                            ...
                            g[a.prop("name")] = a.prop("value")
                });
                for (var h in g)
                    g.hasOwnProperty(h) && g[h] == "all" && delete g[h];
                for (var i = 0; i < f.length; i++)
                    a[f[i]] = g,
                    n(f[i], c, !1, !0)//调用ajax搜索。
            })
        }
          , D = function() {
            $('[data-form="ajax-upload"]').each(function() {
                var a = $(this)
                  ...
                  , f = function(b) {
                    ...
                    a.find(".progressbar .text").html(b + "%"),
                    d = b)
                }
                ;
                l(a, {
                ...
                a.find('[name="upload_option"]').change(function() {
                    ...
                    if (b.prop("checked")) {
                        var c = "disabled";
                        b.val() == "file" ? (a.find('[name="content"]').parents(".file-control").find("input").removeAttr(c).click(),
                        a.find('[name="url"]').attr(c, c).val("").change(),
                        a.find('[name="embed"]').attr(c, c).val("").change().parents(".row").find("label").removeClass("required"),
                        a.find('[name="duration"]').attr(c, c).val("").change().parents(".row").find("label").removeClass("required"),
                        a.find('[name="screenshot"]').parents(".file-control").find("input").attr(c, c).val("").change().parents(".row").find("label").removeClass("required")) : b.val() == "url" ? (a.find('[name="content"]').parents(".file-control").find("input").attr(c, c).val("").change(),
                        a.find('[name="url"]').removeAttr(c).focus(),
                        a.find('[name="embed"]').attr(c, c).val("").change().parents(".row").find("label").removeClass("required"),
                        a.find('[name="duration"]').attr(c, c).val("").change().parents(".row").find("label").removeClass("required"),
                        a.find('[name="screenshot"]').parents(".file-control").find("input").attr(c, c).val("").change().parents(".row").find("label").removeClass("required")) : b.val() == "embed" && (a.find('[name="url"]').attr(c, c).val("").change(),
                        a.find('[name="content"]').parents(".file-control").find("input").attr(c, c).val("").change(),
                        a.find('[name="embed"]').removeAttr(c).focus().parents(".row").find("label").addClass("required"),
                        a.find('[name="duration"]').removeAttr(c).parents(".row").find("label").addClass("required"),
                        a.find('[name="screenshot"]').parents(".file-control").find("input").removeAttr(c).parents(".row").find("label").addClass("required"))
                    }
                });
                var g = {
                    mode: "async",
          ...
          , F = function() {
                    ...
                    if (b && d && e.exists()) {
                        var f = {
                            success: function(a, f) {
                                ...
                                g.innerHTML = f;
                                if (c)
                                    e.find('.item[data-message-id="' + b.message_id + '"]').replaceWith($(g).find('.item[data-message-id="' + b.message_id + '"]'));
                                else {
            ...
            $(document).on("click", ".list-messages [data-edit-message-id]", function(a) {
                a.preventDefault();
                var b = $(this)
                  , c = b.attr("data-edit-message-id");
                if (!!c) {
                    var d = $("#send_message_form");
                    if (d.find('[name="message_id"]').val())
                        return;
          ...
          // 注册验证
          , signUpVerify =function(opts) {
            var testVal = opts.value,
                   field = opts.field; 
            var  regObj = {
                "username":  {
                        "reg": " /^[a-zA-Z\\d]\\w{4,18}[a-zA-Z\\d]$/",
                        "tips": "用户名包含数字、字母、_，不能以_开头或结尾，长度6-20位"
                 },
                 "pass":  {
                        "reg": "/^\\S{6,20}$/",
                        "tips": "密码长度6-20位"
            ...
            if (field == "pass2") {
                regObj["pass2"] = {
                    "reg": "/^" + opts.form.find(":password").val() + "$/",
                    "tips": "两次输入的密码不一致"
                }
            }

            try{
                legal = eval(regObj[field].reg).test(testVal);
                !legal && opts.$errHolder.text(regObj[field].tips).show();
            }catch(err) {};
            return legal;
          }
          , I = [o, p, q, r, s, t, u, v, w, x, y, z, A, B, C, D, E, F, H]; 
